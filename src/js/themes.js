function customTheme() {
	localStorage.setItem("syndicate.theme", "custom");
...
document.addEventListener("DOMContentLoaded", () => {
	loadTheme();
    if(localStorage.getItem("syndicate.theme") == "custom"){
        document.getElementById("customMenu").style.display = "block";
...
function loadTheme() {
	if (localStorage.getItem("syndicate.theme") == "custom") {
		let theme = localStorage.getItem("syndicate.customTheme");
		if (theme) {
...
function changeTheme(name, value) {
	...
	ogStyle = localStorage.getItem("syndicate.customTheme");
	if (ogStyle) {
		ogStyle = JSON.parse(ogStyle);
		ogStyle[name] = value;
		localStorage.setItem("syndicate.customTheme", JSON.stringify(ogStyle));
		loadTheme();
	} else {
		ogStyle = {};
		ogStyle[name] = value;
